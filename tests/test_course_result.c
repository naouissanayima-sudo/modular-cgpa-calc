...
int testCompletedResult()
{
    ...
    CourseResult result = createIncompleteCourseResult(&course); // Use correct signature from handout workflow
...
int testSortBySemester()
{
    ...
    CourseResult results[3] = {
        createIncompleteCourseResult(&courses[0]),
        createIncompleteCourseResult(&courses[1]),
